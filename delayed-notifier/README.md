...
- Для создания уведомления отправьте `POST` запрос на `/notify` со следующим телом:
...
- Для просмотра уведомления (и его статуса) отправьте `GET` запрос на `/notify/{id}`

- Для отмены уведомления отправьте `DELETE` запрос на `/notify/{id}`
