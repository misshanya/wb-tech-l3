...
func (_q *LinkQuery) QueryClicks() *ClickQuery {
	query := (&ClickClient{config: _q.config}).Query()
...
func (_q *LinkQuery) WithClicks(opts ...func(*ClickQuery)) *LinkQuery {
	query := (&ClickClient{config: _q.config}).Query()
...
func (_q *LinkQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Link, error) {
	...
	if query := _q.withClicks; query != nil {
		if err := _q.loadClicks(ctx, query, nodes,
			func(n *Link) { n.Edges.Clicks = []*Click{} },
			func(n *Link, e *Click) { n.Edges.Clicks = append(n.Edges.Clicks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LinkQuery) loadClicks(ctx context.Context, query *ClickQuery, nodes []*Link, init func(*Link), assign func(*Link, *Click)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Link)
	for i := range nodes {
	...
	query.withFKs = true
	query.Where(predicate.Click(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(link.ClicksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.link_clicks
		if fk == nil {
			return fmt.Errorf(`foreign-key "link_clicks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "link_clicks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
